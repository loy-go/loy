...
## Decision
Generators and core systems express requirements in terms of high-level capabilities (e.g. `HTTP`, `Database`, `Cache`, `Queue`, `Telemetry`) rather than specific vendors. The integration registry maps requested capabilities to concrete implementations based on `loy.yaml`.
